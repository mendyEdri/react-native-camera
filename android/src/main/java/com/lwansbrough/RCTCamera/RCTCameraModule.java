...
public class RCTCameraModule extends ReactContextBaseJavaModule {
    ...
    public void captureWithOrientation(final ReadableMap options, final Promise promise, int deviceOrientation) {
        ...
        if (options.hasKey("playSoundOnCapture") && options.getBoolean("playSoundOnCapture") && options.getInt("type") == 1) {
        ...
        if (RCTCamera.getInstance().getViewFinderInstance().isMediaIsVideo()) { // video
            Log.d("MODE", "captureWithOrientation: OPTIONS: " + options);
            RCTCamera.getInstance().getViewFinderInstance().setCameraMediaTypeVideo(true);
            String res = RCTCamera.getInstance().getViewFinderInstance().startCapture();
            if (RCTCamera.getInstance().getViewFinderInstance().isRecording()) {
                Log.d("TAG", "captureWithOrientation: " + res);
                if (res != null) {
                    WritableMap response = new WritableNativeMap();
                    response.putString("data", res);
            ...
            RCTCamera.getInstance().getViewFinderInstance().setCameraMediaTypeVideo(false);
...
//                    camera.startPreview();
    ...
    public void stopCapture(final ReadableMap options, final Promise promise) {
        ...
        promise.resolve(RCTCamera.getInstance().getmUri());
    }
    @ReactMethod
